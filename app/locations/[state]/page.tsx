...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Mobile Apple CarPlay, Android Auto, dashcam and reverse camera installation across ${state.name}. We come to you — home, office or wherever the car is parked. Fixed pricing, same-week availability.`,
    alternates: {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {getActiveServices().map((service) => (
            <div key={service.slug} className="bg-bg-2 border border-white/[0.08] rounded-xl p-6">
              ...
              <p className="font-serif text-2xl text-accent mb-3">From ${service.fromPrice}</p>
              <Link
